...
- `plugins[].version` は `plugin.json` の `version` と一致させる（`claude plugin tag` が不一致を検出する）。**`claude plugin update` は version が上がらないと内容を更新しない** ので、配布物を変えたら必ず上げる
- 同一リポジトリで配る場合は `"source": "./"` でマーケットプレイス自身を指せる（`claude plugin marketplace add <owner>/<repo>` → `claude plugin install <plugin>@<marketplace>` を実機で確認済み。2026-09）
